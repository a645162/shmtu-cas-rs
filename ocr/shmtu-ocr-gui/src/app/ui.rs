...
impl OcrGuiApp {
        ...
        frame.show(ui, |ui| {
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.add_enabled(
                        !self.is_busy,
                        egui::TextEdit::singleline(&mut self.model_dir).desired_width(560.0),
                    );
                    ui.add_space(8.0);
                    if ui
                        .add_enabled(
    ...
    fn draw_preview_panel(&mut self, ui: &mut egui::Ui) {
        ...
        frame.show(ui, |ui| {
            ui.vertical(|ui| {
                ...
                let preview_height = (ui.available_height() - 145.0).max(180.0);
    ...
    fn draw_actions_panel(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        let frame = Frame::group(ui.style())
            .fill(Color32::from_rgb(247, 249, 252))
            .stroke(Stroke::new(1.0, Color32::from_rgb(218, 225, 232)))
            .inner_margin(Margin::same(14));
        ...
        frame.show(ui, |ui| {
            ui.vertical(|ui| {
                ...
                ui.add_enabled(
                    !self.is_busy,
                    egui::TextEdit::singleline(&mut self.captcha_url).desired_width(f32::INFINITY),
                );
                ui.add_space(8.0);
                ...
                if ui
                    ...
                    .clicked()
                {
                    self.send_command(WorkerCommand::ReleaseModel, "正在释放模型...".to_string());
                }
...
impl eframe::App for OcrGuiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                let left_min_width = (ui.available_width() - 280.0).max(480.0);
                ui.columns(2, |columns| {
                    columns[0].set_min_width(left_min_width);
                    self.draw_preview_panel(&mut columns[0]);
                    self.draw_actions_panel(&mut columns[1], ctx);
                });
